...
export default function Header() {
  ...
  const handleLogout = () => {
    ...
    setIsLoggedIn(false);
    setOpenUser(false);
    router.push("/"); // hoặc reload trang: window.location.reload()
  };
      ...
      <div className="max-w-6xl mx-auto flex items-center justify-between px-6 py-4 ">
        ...
        <nav className="flex items-center gap-8 text-sm font-semibold">
          ...
          >
            ...
            {openSearch && (
              <div
                className="absolute right-0 top-12 z-50 w-[320px] bg-white border border-gray-200 shadow-md p-4"
                onMouseEnter={() => setOpenSearch(true)}
                onMouseLeave={() => setOpenSearch(false)}
              >
          ...
          >
            ...
            {openUser && (
              <div
                className="absolute right-0 top-12 w-[200px] z-50 bg-white border border-gray-200 shadow-md p-4"
                onMouseEnter={() => setOpenUser(true)}
                onMouseLeave={() => setOpenUser(false)}
              >
                <div className="absolute -top-2 right-4 w-4 h-4 bg-white border-l border-t border-gray-200 rotate-45"></div>
